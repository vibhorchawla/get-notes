                        ...
                        >
                            <View style={styles.uploaderRow}>
                                <View style={styles.uploaderAvatar}>
                                    {note.uploaderAvatar ? (
                                        <Text style={styles.uploaderAvatarText}>{note.uploaderName?.charAt(0).toUpperCase() || note.uploadedBy?.name?.charAt(0).toUpperCase() || 'A'}</Text>
                                    ) : (
                                        <Text style={styles.uploaderAvatarText}>{(note.uploaderName || note.uploadedBy?.name || 'A').charAt(0).toUpperCase()}</Text>
                                    )}
                                </View>
                                <View style={styles.uploaderInfo}>
                                    <Text style={styles.uploaderName}>{note.uploaderName || note.uploadedBy?.name}</Text>
                                    {(note.uploaderCollege || note.uploadedBy?.college) ? (
                                        <Text style={styles.uploaderCollege}>{note.uploaderCollege || note.uploadedBy?.college}</Text>
                                    ) : null}
                                </View>
                                {(note.uploaderId || note.uploadedBy?.id) && (
                                    <View style={styles.contributorBadge}>
                                        <Ionicons name="ribbon-outline" size={12} color={colors.primary} />
                                        <Text style={styles.contributorBadgeText}>{note.uploaderBadge || 'Contributor'}</Text>
                                    </View>
                                )}
                            </View>
                            <View style={styles.uploaderMeta}>
                                {(note.uploaderBranch || note.uploadedBy?.branch) ? (
                                    <View style={styles.uploaderMetaItem}>
                                        <Ionicons name="git-branch-outline" size={14} color={colors.textSecondary} />
                                        <Text style={styles.uploaderMetaText}>{note.uploaderBranch || note.uploadedBy?.branch}</Text>
                                    </View>
                                ) : null}
                                {(note.uploadedAt || note.createdAt) ? (
                                    <View style={styles.uploaderMetaItem}>
                                        <Ionicons name="calendar-outline" size={14} color={colors.textSecondary} />
                                        <Text style={styles.uploaderMetaText}>{new Date(note.uploadedAt || note.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</Text>
                                    </View>
                                ) : null}
                            </View>
                        </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    contributorBadge: {
        ...
        backgroundColor: 'rgba(91, 127, 255, 0.1)', borderRadius: 999,
